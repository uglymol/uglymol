...
# The most efficient solution that comes to my mind is to replace triTable
# with a table of unique line segments.
#
# To make it complete we'd need to treat sepecially the outer cells
# of the block, which would make it slower and more complicated. So we don't.
#
# N.B. I spent hours browsing through the literature about isosurfaces
# and I think at some point I've seen a paper specifically about MC for
...
# example: idx=1 -> edge [1,2] -> vertices [1,0,0] [1,1,0] -> mid [1, 0.5, 0]
def mid_edge(idx):
    i1, i2 = edgeIndex[idx]
    v1 = cubeVerts[i1]
    v2 = cubeVerts[i2]
    return [(a1 + a2) / 2. for (a1, a2) in zip(v1, v2)]

# input: indexes of edgeIndex, i.e. numbers from triTable
def common_face(a, b):
    faces = [(u if u == v and u in [0, 1] else None)
             for u, v in zip(mid_edge(a), mid_edge(b))]
    assert faces.count(None) in [2, 3]
    return faces

def make_edge_mask(values):
    mask = 0
    for v in values:
        mask |= (1 << v)
    return mask
...
def main():
    total_seg = 0
    new_edge_table = []
    for n, row in enumerate(triTable):
        assert len(row) % 3 == 0
        segments = []
    ...
    for i in range(0, 256, 8):
        print ' '.join('0x%-3x,' % j for j in new_edge_table[i:i+8])
